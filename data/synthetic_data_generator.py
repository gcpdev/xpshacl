...
def generate_complex_shapes(g_shapes):
    ...
    g_shapes.add((ex.SparqlConstraint, sh.message, Literal("Integer value must be even.")))
    g_shapes.add((ex.SparqlConstraint, sh.SPARQLSelectExecutable, Literal("SELECT $this WHERE { FILTER ( ($this % 2) != 0 ) }")))
